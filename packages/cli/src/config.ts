...
export const configPath = () =>
  process.env.KB_CONFIG ?? path.join(process.env.XDG_CONFIG_HOME ?? path.join(os.homedir(), ".config"), "kb", "config.json");
...
export function loadConfig(): Partial<CliConfig> {
  ...
  return {
    url: process.env.KB_URL ?? file.url,
    token: process.env.KB_TOKEN ?? file.token,
  };
}
