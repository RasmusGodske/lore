...
|---|---|---|
| **Bare repository** | The permanent knowledge base. A bare git repo plus its `hooks/` directory. | Host filesystem, e.g. `/srv/kb/knowledge.git` |
...
- `packages/cli` — the `kb` binary: a dependency-free HTTP client whose types are generated
...
they skip themselves unless `KB_TEST_URL` and `KB_TEST_ADMIN_TOKEN` are set, and
...
```
  ...
  │  │ --runtime=runsc    │  │ --runtime=runsc  │  (kb-net, --internal)  │
  │  │ /workspace mounted │  │ /workspace       │                        │
  │  └────────────────────┘  └──────────────────┘                        │
  │                                                                      │
  │  /srv/kb/knowledge.git    ← bare repository, never mounted in sandbox│
  │  /srv/kb/sessions/<id>/   ← per-session working directories          │
  │  /srv/kb/kb.db            ← SQLite state and audit                   │
  └──────────────────────────────────────────────────────────────────────┘
```
...
docker run -d \
  ...
  --memory 1g --cpus 1 \
  --pids-limit 256 \
  -v /srv/kb/sessions/<id>:/workspace \
  -e KB_SESSION_ID=<id> \
  kb-sandbox:latest \
  sleep infinity
```
...
- `KB_SANDBOX_RUNTIME` is a config flag: `runsc` on the VM, `runc` for local development
...
- `--network kb-net` gives reachability to the orchestrator's git endpoint and nothing
  else. See below.

## Network

Sandboxes need exactly one thing from the network: the orchestrator's git endpoint.
`kb-net` is a Docker `--internal` network: the orchestrator and the sandboxes are attached
...
docker run -d \
  ...
  -v /srv/kb:/srv/kb \
  -e KB_SANDBOX_RUNTIME=runsc \
  -p 8080:8080 \
  kb-orchestrator:latest
docker network connect kb-net kb-orchestrator
```

The orchestrator sits on two networks: the default bridge, so `-p 8080:8080` works (an
`--internal` network cannot publish ports), and `kb-net`, where sandboxes reach it as
`kb-orchestrator:8080`. `docker-compose.yml` in the repo does the same for local
development with `KB_SANDBOX_RUNTIME=runc`.
...
**`kb-orchestrator`** — the application: one Node 22 (NestJS) process with session management,
...
**`kb-sandbox`** — the agent's environment, run as uid 1000. Whatever tools the agent
should have:
...
- **Orchestrator restarts with sessions live.** Session state must survive restart. Persist
  it in SQLite at `/srv/kb/kb.db`, not only in memory. On boot, reconcile: any container
